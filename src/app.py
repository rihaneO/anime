...
@st.cache_resource
def _load_ref() -> Referential:
    return Referential(RULES_PATH)
...
@st.cache_resource
def _load_checker(_ref: Referential) -> Checker:
    return Checker(ref=_ref, db=_load_db())


ref = _load_ref()
db = _load_db()
checker = _load_checker(ref)
...
if patient_name:
    try:
        ...
        logger.error("get_history failed for %s: %s", patient_name, exc)
...
with tab_check:
    ...
    if analyze_btn and raw_text:
        try:
            ...
            logger.error("Analyse failed: %s", exc, exc_info=True)
...
with tab_audit:
    ...
    if audit_btn and audit_patient:
        try:
            ...
            logger.error("Audit failed for %s: %s", audit_patient, exc, exc_info=True)
...
with tab_csv:
    ...
    if csv_btn and uploaded:
        try:
            ...
            st.error(f"Erreur lors de l'import CSV : {exc}")
