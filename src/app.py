...
_ref_version = checker.rules.get("schema_version", "?")
_amo = checker.rules.get("lettre_cle", {}).get("valeur_eur", {}).get("metropole")
st.caption(
    f"Référentiel NGAP v{_ref_version} · Valeur AMO métropole : {_amo} € "
    f"· Mis à jour pour l'avenant 21 (en vigueur 23/02/2026)"
)
...
if analyze_btn and raw_text:
    ...
    with st.spinner("Analyse en cours..."):
        ...
        # Montant théorique (coefficient x valeur AMO). Décote -30% si
        # un renouvellement de bilan a moins d'un an est détecté (règle RT1).
